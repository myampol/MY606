---
title: "Lab1 - Introduction to data"
author: "Michael Y."
...
str(cdc)
```
There are __`r numcases`__ cases in the CDC dataset, each of which contains __`r numvariables`__ variables.

The variables __"height"__, __"weight"__, __"wtdesire"__, and __"age"__ are __numeric__ and *should* be considered __continuous__.
However, of these four, only the __"height"__ variable is actually stored in __"numeric"__ format (enabling values of partial inches), while the other three ( __weight__, __wtdesire__, and __age__) are actually stored as integers (precluding partial pounds or years.)  It could be argued that such variables are being handled as _discrete_ rather than _continuous_ in the implementation.

The variables __"exerany"__ ,  __"hlthplan"__ , and __"smoke100"__ are logical variables, each stored as 0 or 1, and are thus __categorical/nominal__ variables.

Similarly, the variable __"gender"__ is a __categorical/nominal__ variable, stored as a factor with levels "m" and "f".

The variable __"genhlth"__ is a __categorical__ variable, stored as a factor with the following levels:

`r levels(cdc$genhlth)`

This is fundamentally an __ordinal__ variable, though it has not been explicitly stored in this dataframe as "ordered."
...
##### Summary and interquartile range for __height__  :
```{r Q2-height}
...
IQR(cdc$height)
```
##### The IQR for height is `r IQR(cdc$height)` .

##### Summary and interquartile range for __age__  :
```{r Q2-age}
...
IQR(cdc$age)
```
##### The IQR for age is `r IQR(cdc$age)` .
...
However, these differences are narrow, as the proportion of males who __have__ smoked more than 100 cigarettes is `r (table(cdc$gender,cdc$smoke100)["m",]/table(cdc$gender)["m"])["1"]` 
while the proportion of females who have __not__ smoked 100 cigarettes is 
`r (table(cdc$gender,cdc$smoke100)["f",]/table(cdc$gender)["f"])["0"]` .
...
```{r Q4-under23-and-smoke}
under23_and_smoke <- subset(cdc, age < 23 & smoke100)    # we don't need "== 1" because that's redundant
```
The number of individuals who are under age 23 and who are smokers is `r nrow(under23_and_smoke)` .
...
##### The box plot of Body Mass Index (BMI) broken out by self-assessed general level of health (above) shows a relationship between __lower BMI__ and __better__ general level of health.

##### Make a boxplot of __BMI__ vs. the binary variable indicating whether or not the subject engages in __exercise__ :

```{r boxplot-exerany}
boxplot(bmi~cdc$exerany)
```
 
##### I chose the variable __"exerany"__ under the hypothesis that individuals who do __not__ engage in exercise 
##### (left-side boxplot, label "0") would have __greater BMI__ than individuals who __do__ engage in exercise 
##### (right-hand plot, label "1")

##### Below, the summary data which is reflected in the above boxplots:

```{r summary-BMI-exerany}
...
```{r scatter-weight-vs-wtdesire, eval=TRUE}
...
##### Individuals plotted above the green line desire to gain weight, while individuals below the green line desire to lose weight.  There are more points below the green line than above.
##### The blue line fits a linear regression through the data.  It has slope `r slope` and intersects the green line at ( x , y ) = (`r intersection`,`r intersection`).  
##### This indicates that, on average, individuals weighing more than `r intersection` desire to lose weight, while individuals weighing less than `r intersection` desire to gain weight.
...
##### `wdiff` is numerical data.  Like __weight__ and __wtdesire__ , it is fundamentally _continuous_ , but because each of __weight__ and __wtdesire__ are implemented as __int__ in the cdc dataframe, their difference is automatically stored as __int__ as well.  This causes such data to exhibit __discrete__ properties unless it is coerced to __numeric__.
##### If an observation `wdiff` is 0, the person's desired weight is the same as his/her current weight. The number of observations for which this is the case is `r sum(as.integer(wdiff==0))` .
##### If an observation `wdiff` is positive, the person wishes to gain weight. The number of observations for which this is the case is `r sum(as.integer(wdiff>0))` .
##### If an observation `wdiff` is negative, the person wishes to lose weight. The number of observations for which this is the case is `r sum(as.integer(wdiff<0))` .
...
##### There are a few outliers, particularly on the men's side, which indicate that the participant wishes to __gain__ a very large amount of weight, e.g, 500 lbs.  It seems quite implausible that someone currently weighing 180 lbs would desire to __increase__ his weight to 680 lbs, which suggests that such data points are erroneous.
...
proportion
```
##### The mean weight is `r meanweight`.  The standard deviation is `r sdweight` .
##### The +/-1 standard deviation range is ( `r lowweight` , `r highweight` ).
##### The number of observations with weight in this range is `r count` .
##### This figure, as a proportion of all observations, is `r proportion` .
